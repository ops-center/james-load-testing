...
install:
	...
	helm upgrade -i james-load-testing charts/james-load-testing --wait \
...
uninstall:
	@cd ../installer; \
	helm uninstall james-load-testing --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/james-load-testing run \
		--v=3 \
		--secure-port=8443 \
